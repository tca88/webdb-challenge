...
1. Explain the difference between `RDBMS` and `SQL`.
   ...
   **_Answer_**
   ...
   RDBMS stands for Relational Database Management System, and is the database software itself, which manages storage, querying and updating. SQL stands for Structured Query Language, and it's only a language which is used to query the RDBMS and output data.

1) Why do tables need a `primary key`?

   **_Answer_**
   Tables need a primary key to let people query each table row individually and modify each row without altering other rows in the same table. It's also a way to reference in other tables as foreign keys so that joining of tables can occur.
...
1) What do we need in order to have a _many to many_ relationship between two tables.
   ...
   **_Answer_**
   Columns in tables containing keys that match up with the keys of other tables. This can be accomplished by a separate table called a join table as one way, for example. Each record in a join table includes a match field that contains the value of the primary keys of the multiple tables it joins.
